"""
FlowRefiner: Flow Matching-Based Iterative Refinement for 3D Turbulent Flow.

Design:
- k=0  : base predictor that regresses the clean next block from zeros input
         via MSE loss (conditioned on the input context).
- k>=1 : flow-matching refinement that predicts the unit-variance velocity
         (= -noise, scaled by sigma_k) along the OT path between the perturbed
         estimate and the clean target.

Sigma schedules (set via SIGMA_SCHEDULE):
- 'fixed_range' : decoupled small-noise range (paper default; sigma_max=0.01,
                  sigma_min=0.001), independent of refinement depth K.
- 'ddpm'        : exponential decay from MIN_NOISE_STD (coupled to K).
- 'ddpm_large'  : same formula with MIN_NOISE_STD=0.01 (larger noise range).
- 'linear'      : linear interpolation between the DDPM endpoints.

Training:
    tau ~ U[0, 1]   for each sample, each refinement step k
    v_target = (y_clean - y_perturbed) / sigma_k = -noise   (unit variance)

Inference:
    N Euler substeps integrate the ODE from tau=0 to tau=1 at each
    refinement level k = K, K-1, ..., 1. N=2 is sufficient in practice.
"""
...
from .unet_backbone import RefinerUNet3D
from utils.noise_generators import generate_noise as _generate_noise


def _compute_ddpm_sigmas(min_noise_std, K):
    """
    Compute FlowRefiner refinement sigmas from a DDPM-derived schedule.
    ...
    # Extract sigmas for refinement (k=1,...,K, decreasing)
...
class FlowMatchingScheduler:
    """
    Flow-matching scheduler for FlowRefiner.

    Supports three sigma schedule types:
    - 'ddpm': Derived from a DDPM cumprod schedule
    ...
    def __init__(self, num_refine_steps=3, min_noise_std=4e-7, sigma_schedule='ddpm',
        ...
        print(f"\n[FlowMatchingScheduler] Initialized:")
...
class FlowRefiner3D(nn.Module):
    """FlowRefiner for 3D turbulence prediction.

    - k=0  : MSE base prediction from zeros input
    - k>=1 : flow-matching velocity matching (predict -noise, scale by sigma_k)

    Loss normalization keeps gradient magnitudes comparable across k:
        k=0  MSE loss  ~ signal_variance * T*C
        k>=1 velocity ~ 1.0 * T*C   (unit variance)
    ...
    def get_training_loss(self, input_dense, output_dense):
        """
        Training loss with normalized velocity targets.
